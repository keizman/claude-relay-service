...
- 📊 **监控面板**: Web界面查看所有数据
- 🛡️ **安全控制**: 访问限制、速率控制
- 🌐 **代理支持**: 支持HTTP/SOCKS5/SOCKS5H代理

> **💡 代理配置说明**：
> - **SOCKS5**: 标准SOCKS5代理，客户端本地解析域名
> - **SOCKS5H**: SOCKS5代理，通过代理服务器解析域名（**推荐**）
> - **HTTP**: HTTP/HTTPS代理
> 
> 🔧 **使用建议**：添加Claude账户时如果SOCKS5连接失败，请尝试使用SOCKS5H，特别是在需要绕过地理限制或DNS解析受限的环境中。
...
1. 点击「Claude账户」标签
2. **（可选）设置代理**: 如果需要使用代理访问Claude服务，请配置代理信息
   - 支持SOCKS5、SOCKS5H、HTTP代理
   - **推荐使用SOCKS5H**：如果SOCKS5连接失败，请尝试SOCKS5H（通过代理解析域名）
3. 点击「添加账户」
...
**注意**: 如果你在国内或网络受限地区，这一步可能需要配置代理。OAuth授权过程也会通过配置的代理进行。
